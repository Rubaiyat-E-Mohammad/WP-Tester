...
class WP_Tester_Crawler {
    ...
    /**
     * Run flow discovery (always generates flows - for Discover Flows button)
     */
    public function run_flow_discovery($include_admin = true) {
        ...
        try {
            ...
            // ALWAYS generate flows for flow discovery (ignore settings)
            $saved_flows_count = $this->process_discovered_flows($discovered_flows, $prevent_duplicates);
            error_log('WP Tester: Flow discovery generated ' . $saved_flows_count . ' flows');
            
            $execution_time = microtime(true) - $start_time;
            
            // Crawl admin panel if requested and setting allows - ALWAYS generate flows for discovery
            $admin_flows = 0;
            ...
            error_log(sprintf(
                'WP Tester: Flow discovery completed. Crawled %d URLs, generated %d frontend flows, %d admin flows in %.2f seconds.',
                count($crawled_urls),
                $saved_flows_count,
                $admin_flows,
                $execution_time
